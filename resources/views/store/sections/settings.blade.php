...
<div id="settings-section" class="p-6 bg-white dark:bg-gray-800 shadow rounded-xl mb-8">
  <div class="mb-6 border-b pb-4 border-gray-200 dark:border-gray-600">
    ...
    <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage your store profile, social links, and branding options.</p>
  </div>
  ...
  <form method="POST" action="{{ route('store.settings.update') }}" enctype="multipart/form-data">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div>
        ...
        <select name="theme" class="w-full px-4 py-2 border rounded-lg dark:bg-gray-900 dark:text-white">
          <option value="light" {{ session('store_theme') === 'light' ? 'selected' : '' }}>🌞 Light</option>
          <option value="dark" {{ session('store_theme') === 'dark' ? 'selected' : '' }}>🌙 Dark</option>
          <option value="auto" {{ session('store_theme') === 'auto' ? 'selected' : '' }}>🌓 Auto</option>
        </select>
      </div>
